...
#define LOAD_PROC(x) GLMLoadProc( x, #x, hOpengl32dll ); \
	if (!x) {iret = FALSE; \
		Msg ("Error loading opengl function "); \
		Msg (#x); \
		Msg ("\n");}
...
#define LOAD_PROC_EXT(x) GLMLoadProc_EXT( x, #x ); \
	if (!x) {iret = FALSE; \
		Msg ("Error loading extension opengl function "); \
		Msg (#x); \
		Msg ("\n");}
