...
{
	V_HSPRITE hsprTexture = LoadSprite("sprites/white.spr");
	if (!hsprTexture){
...
void DumpLeafInfo()
{
	...
	if (!fp)
	{
		Msg("ERROR: couldnt open file %s for writing!\n", buf);
		return;
	}
...
void DumpLevel()
{
	...
	for (int i = 0; i < world->numsurfaces; i++)
	{
		if (!(surfaces[i].flags & (SURF_DRAWSKY|SURF_DRAWTURB)))
		{
			...
			if (p->numverts > 0)
			{
				...
				if (val[0] == -1)
					Msg("warning: bad texture id for %s!\n", surfaces[i].texinfo->texture->name);
	...
	fclose(fp);
	free(polydata);
	Msg("leveldump created (%d textures, %d polys)\n", numtextures, numpolys);
}
