...
void EXT_LightPoint(Vector pos, lighting_ext *lightinfo, int invlight);
...
Vector	g_bdynlightvec[MAX_MODEL_DYNLIGHTS][MAXSTUDIOBONES]; // light vectors in bone reference frames
Vector	g_dynlightcolor[MAX_MODEL_DYNLIGHTS];
int		g_numdynlights;
...
// =================================================================
/*
void CStudioModelRenderer::Lighting (float *lv, int bone, int flags, Vector normal)
{
	float 	illum;
	float	lightcos;
...
// buz: colored
void CStudioModelRenderer::Lighting (Vector &lv, int bone, int flags, Vector normal)
{
	Vector 	illum;
	float	lightcos;
...
void CStudioModelRenderer::Chrome (int *pchrome, int bone, Vector normal)
{
	float n;
	...
	if (g_chromeage[bone] != *m_pStudioModelCount)
	{
		...
		VectorNormalize( tmp );
		CrossProduct2( tmp, m_vRight, chromeupvec );
...
void CStudioModelRenderer::GL_SetupLighting ()
{
	...
	Vector center = (m_pCurrentEntity->curstate.mins + m_pCurrentEntity->curstate.maxs) / 2;
	Vector point = m_pCurrentEntity->origin + center;
...
//	Msg("model: %s\n", m_pRenderModel->name);
	...
	for (int i = 0; i < numlights; i++)
	{
		...
		Vector forward; // light forward for spotlights check
		float spotcos;
...
void CStudioModelRenderer::GL_SetupModel ( int bodypart )
{
	...
	{
		Msg("SetupModel: no such bodypart %d\n", bodypart);
		bodypart = 0;
	}
...
//
	...
	//	Msg("texture %s, flags %d\n", ptex->name, ptex->flags);
		...
		if (ptex->flags & STUDIO_NF_MASKED && !useblending)	// buz
		{
...
//=============================
//int recursioncheck;
int EXT_RecursiveLightPoint (mnode_t *node, const Vector &start, const Vector &end, lighting_ext *lightinfo);
...
void PARSE_COLOR (Vector &out, color24 *lightmap)
{
...
int EXT_RecursiveLightPoint (mnode_t *node, const Vector &start, const Vector &end, lighting_ext *light)
{
...
// check for impact on this node
	...
	for (i=0 ; i<node->numsurfaces ; i++, surf++)
	{
		...
		s = DotProduct2(mid, tex->vecs[0]) + tex->vecs[0][3];
		t = DotProduct2(mid, tex->vecs[1]) + tex->vecs[1][3];
		...
		if (lightmap)
		{
			Vector origcolor = Vector(0, 0, 0);
			Vector ambientcolor = Vector(0, 0, 0);
			Vector diffusecolor = Vector(0, 0, 0);
			Vector lightvec;
			float dot;
			int not_normal = 0;
