...
// 0  - success
// -1 - failed
int RecursiveLightPoint (mnode_t *node, const Vector &start, const Vector &end, Vector &outcolor)
{
	int			r;
...
int R_LightPoint (Vector &p, Vector &outcolor)
{
	Vector		end;
//	int			r;
...
void FrustumCheck::R_SetFrustum (Vector vangles, Vector vorigin, float viewsize)
{
	int i;
	Vector vpn, vright, vup;
...
void FrustumCheck::R_SetFrustum (Vector vangles, Vector vorigin, float view_hor, float view_ver, float dist)
{
	int i;
	Vector vpn, vright, vup;
	...
	Vector farpoint = vorigin + (vpn * dist);
...
qboolean FrustumCheck::R_CullBox (Vector mins, Vector maxs)
{
	int i;
	for (i=0 ; i<4 ; i++)
	{
...
void FrustumCheck::GL_FrustumSetClipPlanes()
{
	...
	else
	{
		...
		Vector points[4]; // up-left, up-right, down-right, down-left
