...
void CClientViewport::InvalidateLayoutForVGUI(vguiMenuID iMenu)
{
	...
	{
	case MENU_TEAM:
		m_pTeamMenu->ReloadLayout();
		m_pTeamMenu->InvalidateLayout();
		break;
	case MENU_MOTD:
		m_pMOTD->ReloadLayout();
		m_pMOTD->InvalidateLayout();
		break;
	case MENU_HTML_MOTD:
		m_pMOTD->ReloadLayout();
		m_pMOTD->InvalidateLayout();
	...
	default:
		ConPrintf(ConColor::Red, "Error: attempted to invalidate unknown VGUI menu: %d\n", iMenu);
		Assert(!("Invalid VGUI menu"));
		break;
	}
}

void CClientViewport::ShowVGUIMenu(vguiMenuID iMenu)
{
	// Don't open menus in demo playback
	if (gEngfuncs.pDemoAPI->IsPlayingback())
		return;
...
bool CClientViewport::IsVGUIVisible(vguiMenuID iMenu)
{
	switch (iMenu)
	{
		case MENU_TEAM: return m_pTeamMenu->IsVisible();
		case MENU_HTML_MOTD:
		case MENU_MOTD: return m_pMOTD->IsVisible();
