...
void Clamp( Vector in, color24 *out )
{
...
void UpdateLightmaps()
{
	...
	if (!gl.IsGLAllowed())
	{
		Msg("ERROR: UpdateLightmaps called when GLmanager not initialized!\n");
		return;
	}
	...
	for (i=0 ; i<world->numsurfaces ; i++)
	{
		...
		{
			Msg("WARNING! surface %d uses lightmap index %d greater than MAX_LIGHTMAPS!\n", i, surf->lightmaptexturenum);
			continue;
		}
	...
	if (needs_extra_lightmaps)
		Msg("Loaded %d x 3 additional lightmaps\n", lightmapstotal+1);
	else
		Msg("Loaded %d additional lightmaps\n", lightmapstotal+1);
}
