...
int g_flashlight = 0; // flashlight attenuation

// sets in model space.
// may be rotated if drawing entity
Vector current_light_origin;
Vector current_spot_vec_forward;
Vector current_light_color;
...
int CullPointlightBBox (Vector mins, Vector maxs)
{
...
void SetupProjection()
{
	...
	Vector target = current_light_origin + (current_spot_vec_forward * 256);
...
int SetupSpotLight()
{
	...
	if (!gl.alphabits)
	{
		Msg("WARNING: no alpha buffer, spotlights will not be drawn!\n");
		return FALSE;
	}
...
//		return FALSE;
	...
	if (!gl.alphabits)
	{
		Msg("WARNING: no alpha buffer, pointlights will not be drawn!\n");
		return FALSE;
	}
...
{
	//	Msg("Paranoia dynamic light features are not supported by your video card!\n");
	...
	for (int i=0 ; i<MAX_DLIGHTS ; i++, dl++)
	{
		...
		if (current_light->spot_texture[0]) // spotlight
		{
			...
			Vector lightangles = current_light->angles;
...
Vector plightcol = Vector(1.0f, 1.0f, 1.0f); // color of 3d attenuation texture
...
int Create3DAttenuationTexture()
{
	...
	if (!buf)
	{
		Msg("Error: Failed to allocate memory for 3d attenuation texture!\n");
		return 0;
	}
