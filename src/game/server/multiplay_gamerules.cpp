...
void CHalfLifeMultiplay::DeathNotice(CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pevInflictor)
{
	...
	if (pVictim->pev == pKiller)
	{
		...
		if ( pVictim->m_flSuicideTimer == -1 )
			pVictim->GiveAchievement( KILLYOSELF );
	...
	else if (pKiller->flags & FL_CLIENT)
	{
		...
		if ( pAttacker && pAttacker->IsPlayer() )
			pVictim->GiveAchievementsFromKill( pAttacker, killer_weapon_name );
	}
	else
	{
		// The player killed themselves, give em this stupid ass achievement.
...
{
	if (pWeapon && pWeapon->GetWeaponID() && (pWeapon->iFlags() & ITEM_FLAG_LIMITINWORLD))
	{
...
BOOL CHalfLifeMultiplay::CanHavePlayerItem(CBasePlayer *pPlayer, CBasePlayerItem *pItem)
{
	if (weaponstay.value > 0)
	{
		...
		for (int i = 0; i < MAX_ITEM_TYPES; i++)
		{
			...
			while (it != NULL)
			{
				if (it->GetWeaponID() == pItem->GetWeaponID())
				{
					return FALSE;
				}
