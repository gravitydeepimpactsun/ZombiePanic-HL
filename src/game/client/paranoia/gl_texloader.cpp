...
int CreateTexture(const char* filename, int mipmaps, int useid)
{
	...
	if (!gl.IsGLAllowed())
	{
		Msg("CreateTexture error: glmanager is not initialized!\n");
		return 0;
	}
	...
	{
		Msg("Error: texture %s uses unsupported data format -->\n(only 24-bit noncompressed TGA supported)\n", filename);
	...
	{
		Msg("Error: texture %s: sizes %d x %d are not supported by your hardware\n", filename, hdr->Width, hdr->Height);
		gEngfuncs.COM_FreeFile(file);
		return 0;
	}
	...
	{
		Msg("Error: texture %s has bad dimensions (%d x %d)\n", filename, hdr->Width, hdr->Height);
		gEngfuncs.COM_FreeFile(file);
		return 0;
	}
	...
	{
		Msg("Error: texture %s (%d x %d) doesn't fit in static buffer\n", filename, hdr->Width, hdr->Height);
		gEngfuncs.COM_FreeFile(file);
		return 0;
	}
...
int Create_DSDT_Texture(const char* filename)
{
	...
	if (!gl.IsGLAllowed())
	{
		Msg("Create_DSDT_Texture error: glmanager is not initialized!\n");
		return 0;
	}
...
int ParseDetailTextureDataFile()
{
	...
	{
		Msg("ERROR: ParseDetailTextureDataFile() cant get level name!\n");
		return FALSE;
	}
	...
	while(1)
	{
		...
		{
			Msg("Too many entries in detail textures file %s\n", levelname);
			break;
		}
		...
		{
			Msg("Error in detail textures file %s: token too large\n", levelname);
...
int GetDetailTextureData(char *_texname, int &glid, int &x_scale, int &y_scale)
{
	...
	if (!glid)
	{
		Msg("Detail texture %s not found!\n", path);
		return FALSE;
	}
...
int LoadCacheTexture(const char* filename, int mipmaps, int clamped)
{
	...
	{
		Msg("Error: LoadCacheTexture failed: MAX_CACHED_TEXTURES exceeded!\n");
		return 0;
	}
