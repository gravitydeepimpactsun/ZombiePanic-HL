...
float GetMaxPossibleSpeed( ZPPlayerMovementDirection_t dir )
{
	...
	switch (dir)
	{
		...
		case DirBack: flValue -= 20; break;
		case DirSides: flValue -= 10; break;
	}
	return flValue;
}
// ZOMBIE PANIC - END
...
void CL_DLLEXPORT CL_CreateMove(float frametime, struct usercmd_s *cmd, int active)
{
	...
	if (active)
	{
		...
		if (in_strafe.state & 1)
		{
			cmd->sidemove += GetMaxPossibleSpeed( ZPPlayerMovementDirection_t::DirSides ) * CL_KeyState(&in_right);
			cmd->sidemove -= GetMaxPossibleSpeed( ZPPlayerMovementDirection_t::DirSides ) * CL_KeyState(&in_left);
		}

		cmd->sidemove += GetMaxPossibleSpeed( ZPPlayerMovementDirection_t::DirSides ) * CL_KeyState(&in_moveright);
		cmd->sidemove -= GetMaxPossibleSpeed( ZPPlayerMovementDirection_t::DirSides ) * CL_KeyState(&in_moveleft);
		...
		if (!(in_klook.state & 1))
		{
			cmd->forwardmove += GetMaxPossibleSpeed( ZPPlayerMovementDirection_t::DirForward ) * CL_KeyState(&in_forward);
			cmd->forwardmove -= GetMaxPossibleSpeed( ZPPlayerMovementDirection_t::DirBack ) * CL_KeyState(&in_back);
		}
...
void InitInput(void)
{
	...
	gEngfuncs.pfnAddCommand("+unload", IN_UnloadDown);
	gEngfuncs.pfnAddCommand("-unload", IN_UnloadUp);
	...
	cl_forwardspeed = gEngfuncs.pfnRegisterVariable("cl_forwardspeed", "200", 0);
	cl_backspeed = gEngfuncs.pfnRegisterVariable("cl_backspeed", "160", 0);
	cl_sidespeed = gEngfuncs.pfnRegisterVariable("cl_sidespeed", "180", 0);
