...
Vector skyangles = Vector(0, -90, 90);
...
void CheckSky()
{
	...
	if (!skyname)
	{
		Msg("Sky error: cant get sv_skyname value\n");
		Msg("Use gl_customsky 1 to reenable sky\n");
	...
	for (int i = 0; i < 6; i++)
	{
		...
		if (!ret)
		{
			Msg("Sky error: cant load sky face %s\n", path);
			Msg("Use gl_customsky 1 to reenable sky\n");
			gEngfuncs.Cvar_SetValue( "gl_customsky", 0 );
			return;
		}
...
//		return; // no sky visible
	...
	Vector trans = render_origin - sky_origin;
	if (sky_speed)
