...
const GLubyte * sys_glGetString (GLenum name)
{
	if (name == 0x1F03) // GL_EXTENSIONS
	{
		...
		strcat(extstring, " " RENDER_VERSION );
...
// buz
int sys_wglChoosePixelFormat(HDC hdc, PIXELFORMATDESCRIPTOR *pfd)
{
//	int iret;
//	add_log("color bits: %d\n", pfd->cColorBits);
//	pfd->cColorBits = 32;
//	pfd->cAlphaBits = 8;
//	iret = orig_wglChoosePixelFormat(hdc, pfd);
//	if (iret == 0) // no alpha buffer?..
//	{
//		pfd->cAlphaBits = 0;
//		iret = orig_wglChoosePixelFormat(hdc, pfd);
//	}
