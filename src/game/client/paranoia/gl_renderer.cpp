...
extern Vector	render_origin;
extern Vector	render_angles;


float	savedMinmaxs[6];
int		renderer_prepared_for_this_frame = 0;
Vector	saved_view_orig;
int		newframe = 0;
...
void DisableWorldDrawing( ref_params_t *pparams )
{
	...
	const Vector vieworg(pparams->vieworg);
	const Vector forward(pparams->forward);
	VectorMA(vieworg, -100, forward, wcoord);
...
void getCubeVector ( int side, int x, int y, Vector &v )
{
...
//	int			j, k;
	...
	if (e->curstate.renderfx == kRenderFx3DSkyBox) // skybox entity
	{
		if (!gl.RenderVersion)
			return;
		...
		if (sky_speed)
		{
			...
			Vector skypos = sky_origin + (render_origin - sky_world_origin) / sky_speed;
...
void PrepareRenderer( ref_params_t *pparams )
{
	...
	if (!gl.RenderVersion)
	{
		Msg("Render version returned 0, custom renderer disabled\n");
		gEngfuncs.Cvar_SetValue( "gl_renderer", 0 );
		return;
	}
	...
	{
		Msg("No lightdata, custom renderer disabled\n");
		gEngfuncs.Cvar_SetValue( "gl_renderer", 0 );
		return;
	}
	...
	{
		Msg("Paranoia custom renderer is not supported by your videocard\n");
		gEngfuncs.Cvar_SetValue( "gl_renderer", 0 );
		return;
	}
...
void RendererCreateEntities()
{
	...
	if (ztrick)
	{
		Msg("gl_ztrick 1 is not allowed\n");
...
void RendererUpdateEntityList()
{
	...
	if (!localPlayer)
	{
		Msg("ERROR: UpdateEntityList - cant get local player!\n");
		return;
	}
...
//	CONPRINT("RendererDrawNormal\n");
	...
	//	CONPRINT("newframe!\n");
		...
		if (gl.IsGLAllowed() && gl.RenderVersion)
		{
			ResetRenderState();
			DrawSky();
		}
